...
README 的“模板数组类型”定义了 `vector<T>` 和嵌套 `vector<vector<T>>` 的脚本语法与值语义。
本后端仅补充其实现细节：VM 使用共享底层存储和写时复制（COW），只读函数参数传递共享
`VmArray` 存储而不复制全部元素；函数内写入时先分离共享存储，调用者值保持不变。
这属于内部常量共享优化，不是脚本层可观察的引用参数。
