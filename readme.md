...
调用时必须存在参数个数完全相同的版本；例如只定义了 `label(value)` 时，调用 `label()` 或 `label(1, 2)` 会报告可用的参数个数。参数类型不参与重载选择，因此两个同名且参数个数相同的脚本函数会产生静态错误。
...
函数调用的词法解析只会为尚未出现的函数名建立待解析占位，不会覆盖已有函数定义。后续脚本若再次定义同名、同参数个数的函数，仍会报告重复函数定义错误。
...
- 函数名是否存在、函数定义是否重复、是否与宿主函数重名，以及新函数定义时可确定的未初始化变量，属于静态检查。
