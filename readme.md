...
数组可以使用 `vector<T>` 指定元素类型，`T` 可以继续是另一个 `vector<...>`，因此支持嵌套模板：

```c
vector<int> values = {1.9, 2.1};
vector<vector<int>> matrix = {{3.8, 4.2}, {5.7}};

values.push_back(6.9);  // 按 vector<int> 转换为 6
matrix[1][0] = 8.6;     // 按内层 vector<int> 转换为 8
```

这里的 `int` 是 Cifa 的逻辑元素类型：Direct Cifa 的数组仍由 `Object` 元素组成，Bytecode 的数组仍由
`VmArray`/`CompactValue` 元素组成；两条路径中的整数 payload 都统一保存为 `int64_t`，并不是原生的
`std::vector<int>` 或 `std::vector<int64_t>`。

显式元素类型会约束初始化、`push_back`、索引赋值和嵌套元素转换。未初始化的模板数组为空数组：
...
函数定义暂时不需要写模板参数类型，函数参数从调用时的实际数组值获得；脚本语义仍然是按值传递，
函数内修改不会写回调用者数组。字节码后端可以在不改变这个语义的前提下共享数组底层存储，
具体实现见 [cifabytecode.md](cifabytecode.md)。
...
使用 `{}` 花括号构造未指定元素类型的数组，元素之间用逗号分隔。元素类型可以混合（数字、字符串等）：
...
一般情况下，不写变量名的类型即可满足脚本需求。未声明类型的变量和函数形参保持动态值语义，
写法更灵活，也不需要为普通配置或业务逻辑增加额外类型声明。只有在性能敏感的代码路径中，
才建议显式固定类型，例如使用 `int`、`double` 或 `vector<int>`；字节码后端可以据此减少运行时
类型判断，并为局部数值、数组元素和索引访问选择更专门的执行路径。固定类型是优化提示和约束，
不是一般脚本功能的必要条件。
