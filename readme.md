...
5. **运算符归约**  
    ...
    未来可以考虑把这里改为 precedence climbing、Pratt parser 或 shunting-yard 这类一趟表达式解析方案，并继续生成相同形状的 `CalUnitType::Operator` AST。不过当前解析管线是在平铺 token 列表上做后处理归约，旧方案不需要显式计算表达式范围；一趟表达式解析若只替换 `combine_ops()`，需要准确区分 `return` 后表达式、`for` 头、range-for 的 `:`、三元 `?:`、数组赋值左值等边界。这个改动更适合作为较大范围的语句/表达式解析重构，而不是局部性能补丁。
