...
优化默认关闭；在编译前调用 `set_optimization_enabled(true)` 开启。优化后的 Module 允许改变内部指令布局和运行时调用栈，但保持脚本结果与运行时错误语义。
...
开启优化后，宿主函数注册表必须保持不变。每个优化 Module 会在编译时记录宿主函数版本；后续任意 `register_function`（包括覆盖已有名称）都会令它拒绝执行并报告运行时错误。应在注册全部宿主函数后开启编译；需要增删宿主函数时，重新创建并编译 `CifaBytecode`。即使是在编译前覆盖了同名内建函数，该名称也不再参与内建折叠，仍正常调用宿主实现。

开启优化也会冻结 VM 脚本函数目录：同一 Module 可重复执行，但不能在执行期间用 `run_string` 或 `run_file` 发布新的脚本函数。遇到该调用会在执行前报告 `script functions changed during optimized bytecode execution`。这使脚本函数常量折叠不会静默绑定被后续重定义替换的函数体。
